...
class Weather(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.yahoo_url = kwargs.get('yahoo_url', 'http://weather.yahooapis.com/forecastrss?w=2356566')
        self.wu_url = kwargs.get('wu_url', 'http://rss.wunderground.com/auto/rss_full/NC/Asheville.xml?units=english')
        self.get_wu_weather()
    ...
    def get_wu_weather(self):
        ...
        self.r.set('avl_weather:current:temp', conditions[0].split(': ')[1].split('&')[0].strip())
        self.r.set('avl_weather:current:cond', conditions[3].split(': ')[1].split('&')[0].lower().strip())
        self.r.set('avl_weather:current:wind_direction', conditions[4].split(': ')[1].split('&')[0].strip())
        self.r.set('avl_weather:current:wind_speed', conditions[5].split(': ')[1].split('&')[0].strip())
